...
router.post('/generate', async (req, res) => {
  try {
    ...
    const currentSeason = await Season.getCurrentSeason(cowboys.team_id);
    if (!currentSeason) return res.status(404).json({ error: 'No season data' });
    ...
    // ✅ FIX: Build a real record object
    const record = {
    ...
    const prediction = PredictionEngine.calculatePrediction(record, seasonStats);
